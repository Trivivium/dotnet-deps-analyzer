...
namespace App.Rendering
{
    internal class MetricsTableView : StackLayoutView
    {
        ...
        private static IReadOnlyList<MetricsTableLine> GetItems(ProjectInspectionResult project, ITableFilter filter)
        {
            ...
            // Create table lines for each explicit package and its transient dependencies.
            foreach (var root in project.PackageResults.Where(result => result.Package.ReferenceType == PackageReferenceType.Explicit))
            {
            ...
            foreach (var (package, metrics) in project.PackageResults.Where(result => result.Package.ReferenceType == PackageReferenceType.Unknown))
            {
        ...
        private static List<MetricsTableLine> GetTableLines(ProjectInspectionResult project, PackageInspectionResult result, ITableFilter filter, int depth)
        {
            ...
            if (filter.IsPackageExcluded(package))
            {
                return lines;
            }
            ...
            var children = project.PackageResults.Where(r => r.Package.Parents?.Any(parent => parent.ID == package.ID) ?? false);
